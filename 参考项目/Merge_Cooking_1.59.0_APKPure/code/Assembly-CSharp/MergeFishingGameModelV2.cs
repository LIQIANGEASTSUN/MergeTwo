...
public class MergeFishingGameModelV2 : BaseModel
{
	private readonly string mIndependentMergeKey = "숭끿䚋䤮䵱\r\u001e匝숤끲嚐㈥即\r^卝숬끳䮗䔭彻\r2匱뻅";

	public MergeFishingGameModelVoV2 mIndependentMergeVo;
	...
	public override void InitModel(bool timingSave = false)
	{
		...
		mIndependentMergeVo = new MergeFishingGameModelVoV2();
		string item = GetItem(mIndependentMergeKey);
		if (!string.IsNullOrEmpty(item))
		{
			mIndependentMergeVo = JsonConvert.DeserializeObject<MergeFishingGameModelVoV2>(item);
		}
		ReloadConfig();
		TomorrowResetData();
	...
	private void InitConfig()
	{
		...
		List<IIndependentGoodsBaseVo> list = MergeFishingV2.ConfigHelper.fishGoodsList.list;
	...
	public IndGoodsBase GetIndGameBaseById(int id, IndependentGrid grid, IndependentGameGoodsVo initialVo = null, List<int> mergeTotalDrops = null, bool addMergeDrop = false, List<BaseData2VO> mergeItemDrops = null)
	{
		...
		if (addMergeDrop)
		{
			IIndependentGoodsMergeVo item = MergeFishingV2.ConfigHelper.fishGoodsMergeMap.GetItem(independentGameGoodsVo.goodsID);
			if (item != null)
			{
	...
	{
		int openIapTag = mIndependentMergeVo.OpenIapTag3;
		if (mIndependentDropConfig != null && mIndependentDropConfig.InIapTag3(openIapTag))
		{
			return;
		}
		mIndependentDropConfig = null;
		List<IIndependentDropConfigVo> list = MergeFishingV2.ConfigHelper.fishDropList.list;
		for (int i = 0; i < list.Count; i++)
		{
	...
	public void OpenMergeFishing()
	{
		...
		mIndependentMergeVo.OpenIapTag3 = ObjectManager.GetInstance().userTagModel.GetUserIap3Tag();
		ReloadConfig();
	...
	private void InitialIndependentMap()
	{
		...
		if (MergeFishingV2.ConfigHelper.fishGameInitMap == null)
		{
			return;
		}
		List<IGameInitVo> list = MergeFishingV2.ConfigHelper.fishGameInitMap.list;
		...
		MergeFishingV2.IThemeVO currentTheme = ObjectManager.GetInstance().mergeFishingV2Model.GetCurrentTheme();
	...
	private void IndependentSettleup()
	{
		...
		foreach (KeyValuePair<int, IndependentGameGoodsVo> item in mIndependentMergeVo.IndependentMap)
		{
			...
			independentGoodsCollectVo = MergeFishingV2.ConfigHelper.fishGoodsCollectMap.GetItem(independentGoodsBaseVo.goodsID);
		...
		ObjectManager.GetInstance().itemDataModel.AddRewardBaseData3(mSettleRewardList, notice: false, 103);
	}
	...
	{
		if (ObjectManager.GetInstance().mergeFishingV2Model.IsInActivity())
		{
	...
	{
		return ObjectManager.GetInstance().mergeFishingV2Model.GetActivityTimedown();
	}
	...
	public List<int> GenerateProduceList(int goodsId)
	{
		...
		IIndependentProduceVo item = MergeFishingV2.ConfigHelper.fishGoodsProduceMap.GetItem(goodsId);
		if (item == null)
		{
			return result;
		}
	...
	{
		for (int i = 1; i <= MergeFishingV2MainViewLogic_def.FishRows; i++)
		{
			for (int j = 1; j <= MergeFishingV2MainViewLogic_def.FishColumns; j++)
			{
				int key = i * 10 + j;
	...
	public bool AddGoodsToChessboard(int goodsID, Transform startTransform, bool showEnoughTip = false)
	{
		...
		if (HasEmptyGrid())
		{
			EventManager.GetInstance().DispatchEvent<int, Transform>(202059, goodsID, startTransform);
			return true;
		}
		if (showEnoughTip)
		{
	...
	public bool IsHaveSettleReward()
	{
	...
	public void OpenSettleRewardView()
	{
		if (mSettleRewardList != null && mSettleRewardList.Count > 0)
		{
			MergeFishingV2SettleView_defData data = new MergeFishingV2SettleView_defData
			{
				SettleData3Vos = mSettleRewardList
			};
			ViewManager.GetInstance().ShowView<MergeFishingV2SettleView_defLogic>(data, ViewManager.ViewLayer.POP, animate: true);
		}
	}
	...
	public int GetProduceDropGoodsID()
	{
		...
		if (!ObjectManager.GetInstance().mergeFishingV2Model.HasCurrentThemeRes())
		{
			return 0;
		}
	...
	private void PlayDropGoodsAnim(Transform starTransform, string iconName)
	{
		...
		{
			num = 1,
			endFlyType = RewardFlyIconType.MergeFishingV2Activity,
			iconName = iconName,
			iconScale = 0.45f,
			toScale = 0.8f,
	...
	{
		ObjectManager.GetInstance().mergeFishingV2Model.OnAddToken(goodsId, goodsLv, num);
		MergeFishingV2.TA.GetFishToken(goodsId, goodsLv, num, getType);
	}
	...
	public void SyncServerDataToLocal()
	{
		...
		else
		{
			try
			{
				MergeFishingGameModelVoV2 mergeFishingGameModelVoV = JsonConvert.DeserializeObject<MergeFishingGameModelVoV2>(serverItem);
				if (mergeFishingGameModelVoV != null)
				{
					mIndependentMergeVo = mergeFishingGameModelVoV;
				}
			}
			catch (Exception)
			{
	...
	public MergeFishingGameModelVoV2 GetIndependentMergeVo()
	{
	...
	public void DebugClearGrid()
	{
		...
		SaveIndependentMapData();
		EventManager.GetInstance().DispatchEvent(202060);
	}
