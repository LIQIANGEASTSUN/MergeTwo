
undefined4 FUN_031ea0c4(void)

{
  int iVar1;
  undefined4 uVar2;
  undefined4 uVar3;
  int iVar4;
  bool bVar5;
  undefined8 uVar6;
  undefined4 uStack_48;
  undefined4 uStack_44;
  undefined4 uStack_40;
  undefined4 uStack_3c;
  undefined4 uStack_38;
  undefined4 uStack_30;
  undefined4 uStack_2c;
  undefined4 uStack_28;
  undefined4 uStack_24;
  undefined4 uStack_20;
  
  iVar1 = func_0x02953fd4(0x1ae7,0);
  if (iVar1 != 0) {
    iVar1 = func_0x029540a4(0x1ae7,0);
    if (iVar1 == 0) {
      func_0x014388e4();
    }
    uStack_2c = *(undefined4 *)((undefined1  [16])0x0 + (undefined1  [16])0x4);
    uStack_28 = *(undefined4 *)((undefined1  [16])0x0 + (undefined1  [16])0x8);
    uStack_24 = *(undefined4 *)((undefined1  [16])0x0 + (undefined1  [16])0xc);
    uVar2 = 0;
    uStack_30 = 0;
    uStack_20 = 0;
    func_0x024f56c0(&uStack_48,0);
    uStack_30 = uStack_48;
    uStack_2c = uStack_44;
    uStack_28 = uStack_40;
    uStack_24 = uStack_3c;
    uStack_20 = uStack_38;
    if (*(int *)(iVar1 + 0x10) != 0) {
      func_0x01523a6c(&uStack_30,*(int *)(iVar1 + 0x10),0);
      uVar2 = 0;
      if (*(int *)(iVar1 + 0x10) != 0) {
        uVar2 = 1;
      }
    }
    iVar4 = *(int *)(iVar1 + 8);
    uVar3 = *(undefined4 *)(iVar1 + 0xc);
    if (iVar4 == 0) {
      func_0x014388e4();
    }
    func_0x024f56d0(iVar4,uVar3,&uStack_30,uVar2,0,0);
    uVar2 = func_0x024f56e0(&uStack_30,0,0);
    return uVar2;
  }
  iVar1 = FUN_031e695c();
  if (iVar1 == 0) {
    func_0x014388e4();
  }
  iVar1 = FUN_031e7c60(iVar1);
  uVar2 = 0;
  if (iVar1 != 0) {
    iVar1 = FUN_031e695c();
    if (iVar1 == 0) {
      func_0x014388e4();
    }
    FUN_031e6094(iVar1);
    uVar6 = func_0x031e7d54();
    iVar1 = (int)((ulonglong)uVar6 >> 0x20);
    bVar5 = (int)uVar6 != 0;
    if ((int)-(iVar1 + (uint)bVar5) < 0 == (SBORROW4(0,iVar1) != SBORROW4(-iVar1,(uint)bVar5))) {
      iVar1 = FUN_031e695c();
      if (iVar1 == 0) {
        func_0x014388e4();
      }
      iVar1 = FUN_031e6094(iVar1);
      if (iVar1 == 0) {
        func_0x014388e4();
      }
      if (*(char *)(iVar1 + 0x31) == '\0') {
        iVar1 = FUN_031e695c();
        if (iVar1 == 0) {
          func_0x014388e4();
        }
        iVar1 = FUN_031e6094(iVar1);
        if (iVar1 == 0) {
          func_0x014388e4();
        }
        if (*(char *)(iVar1 + 0x30) != '\0') {
          iVar1 = FUN_031e695c();
          if (iVar1 == 0) {
            func_0x014388e4();
          }
          iVar1 = FUN_031e6094(iVar1);
          if (iVar1 == 0) {
            func_0x014388e4();
          }
          uVar2 = 1;
          func_0x026b13e4(iVar1,1,0);
          func_0x031e9348();
        }
      }
    }
  }
  return uVar2;
}

