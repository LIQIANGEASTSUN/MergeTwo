
void FUN_01ccd56c(int param_1)

{
  int iVar1;
  undefined4 extraout_r1;
  undefined4 extraout_r1_00;
  undefined4 extraout_r1_01;
  undefined4 extraout_r1_02;
  undefined4 extraout_r1_03;
  undefined4 uVar2;
  int iVar3;
  undefined4 uVar4;
  undefined4 *puVar5;
  undefined4 uVar6;
  undefined8 uVar7;
  undefined4 uStack_48;
  undefined4 uStack_44;
  undefined4 uStack_40;
  undefined4 uStack_3c;
  undefined4 uStack_30;
  undefined4 uStack_2c;
  undefined4 uStack_28;
  undefined4 uStack_24;
  
  iVar1 = func_0x02953fd4(0xafe9,0);
  if (iVar1 != 0) {
    iVar1 = func_0x029540a4(0xafe9,0);
    if (iVar1 == 0) {
      func_0x014388e4();
    }
    uStack_2c = *(undefined4 *)((undefined1  [16])0x0 + (undefined1  [16])0x4);
    uStack_28 = *(undefined4 *)((undefined1  [16])0x0 + (undefined1  [16])0x8);
    uStack_24 = *(undefined4 *)((undefined1  [16])0x0 + (undefined1  [16])0xc);
    uStack_30 = 0;
    func_0x024f56c0(&uStack_48,0,0);
    uStack_30 = uStack_48;
    uStack_2c = uStack_44;
    uStack_28 = uStack_40;
    uStack_24 = uStack_3c;
    if (*(int *)(iVar1 + 0x10) != 0) {
      func_0x01523a6c(&uStack_30,*(int *)(iVar1 + 0x10),0);
    }
    func_0x01523a6c(&uStack_30,param_1,0);
    iVar3 = *(int *)(iVar1 + 8);
    uVar4 = *(undefined4 *)(iVar1 + 0xc);
    iVar1 = *(int *)(iVar1 + 0x10);
    if (iVar3 == 0) {
      func_0x014388e4();
    }
    uVar2 = 2;
    if (iVar1 == 0) {
      uVar2 = 1;
    }
    func_0x024f56d0(iVar3,uVar4,&uStack_30,uVar2,0,0);
    return;
  }
  iVar1 = func_0x015fcde8(*(undefined4 *)(param_1 + 0x10),0);
  if (iVar1 == 0) {
    iVar1 = FUN_01ccc5a4(param_1);
    if (iVar1 == 0) {
      func_0x014388e4();
    }
    puVar5 = (undefined4 *)(param_1 + 0x20);
    iVar1 = func_0x01523ccc(*(undefined4 *)(iVar1 + 8),*(undefined4 *)(param_1 + 0x10),0);
    uVar7 = FUN_01ccc5a4(param_1);
    uVar4 = (undefined4)((ulonglong)uVar7 >> 0x20);
    iVar3 = (int)uVar7;
    if (iVar1 == 0) {
      if (iVar3 == 0) {
        func_0x014388e4();
      }
      func_0x0268be6c(iVar3,0);
      iVar1 = FUN_01ccc5a4(param_1);
      uVar4 = *(undefined4 *)(param_1 + 0x10);
      if (iVar1 == 0) {
        func_0x014388e4();
      }
      func_0x026a6fd8(iVar1,uVar4,0);
      uVar7 = FUN_01ccc5a4(param_1);
      uVar4 = (undefined4)((ulonglong)uVar7 >> 0x20);
      uVar2 = *(undefined4 *)(param_1 + 0x18);
      uVar6 = *(undefined4 *)(param_1 + 0x1c);
      if ((int)uVar7 == 0) {
        func_0x014388e4();
        uVar4 = extraout_r1_02;
      }
      uStack_28 = 0;
      func_0x026a7168((int)uVar7,uVar4,uVar2,uVar6);
      uVar7 = FUN_01ccc5a4(param_1);
      uVar4 = (undefined4)((ulonglong)uVar7 >> 0x20);
      uVar6 = *puVar5;
      uVar2 = *(undefined4 *)(param_1 + 0x24);
      if ((int)uVar7 == 0) {
        func_0x014388e4();
        uVar4 = extraout_r1_03;
      }
      uStack_28 = 0;
      func_0x026a7238((int)uVar7,uVar4,uVar6,uVar2);
    }
    else {
      uVar6 = *(undefined4 *)(param_1 + 0x18);
      uVar2 = *(undefined4 *)(param_1 + 0x1c);
      if (iVar3 == 0) {
        func_0x014388e4();
        uVar4 = extraout_r1;
      }
      uStack_28 = 0;
      func_0x026a7168(iVar3,uVar4,uVar6,uVar2);
      uVar7 = FUN_01ccc5a4(param_1);
      uVar4 = (undefined4)((ulonglong)uVar7 >> 0x20);
      uVar6 = *puVar5;
      uVar2 = *(undefined4 *)(param_1 + 0x24);
      if ((int)uVar7 == 0) {
        func_0x014388e4();
        uVar4 = extraout_r1_00;
      }
      uStack_28 = 0;
      func_0x026a7238((int)uVar7,uVar4,uVar6,uVar2);
      iVar1 = FUN_01ccc5a4(param_1);
      if (iVar1 == 0) {
        func_0x014388e4();
      }
      if (*(char *)(iVar1 + 0xc) != '\0') {
        uVar7 = FUN_01ccc5a4(param_1);
        uVar4 = (undefined4)((ulonglong)uVar7 >> 0x20);
        uVar6 = *puVar5;
        uVar2 = *(undefined4 *)(param_1 + 0x24);
        if ((int)uVar7 == 0) {
          func_0x014388e4();
          uVar4 = extraout_r1_01;
        }
        uStack_28 = 0;
        func_0x026a73d8((int)uVar7,uVar4,uVar6,uVar2);
      }
    }
  }
  return;
}

