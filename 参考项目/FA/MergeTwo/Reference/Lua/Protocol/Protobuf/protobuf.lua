local wire_format = require "Protocol.Protobuf.wire_format"
local type_checkers = require "Protocol.Protobuf.type_checkers"
local encoder = require "Protocol.Protobuf.encoder"
local decoder = require "Protocol.Protobuf.decoder"
local listener_mod = require "Protocol.Protobuf.listener"
local containers = require "Protocol.Protobuf.containers"
local descriptor = require "Protocol.Protobuf.descriptor"
local FieldDescriptor = descriptor.FieldDescriptor
local text_format = require "Protocol.Protobuf.text_format"
...
local function make_descriptor(name, descriptor, usable_key)
    ...
    meta.__call = function()
        return setmetatable({}, meta)  -- 返回一个全新的容器，类似于一个构造器
    end
...
local function _AddMergeFromStringMethod(message_descriptor, message_meta)
    ...
    local _internal_parse = function(self, buffer, pos, pend)
        ...
        local field_decoder
        while pos ~= pend do
            tag_bytes, new_pos = ReadTag(buffer, pos, pend)
    ...
    local merge_from_string_length = function(self, serialized, length)
        if _internal_parse(self, serialized, 0, length) ~= length then
            error('Unexpected end-group tag.')
        end
        return length
    end
    message_meta._member.ParseFromStringLength = function(self, serialized, length)
        message_meta._member.Clear(self)
        merge_from_string_length(self, serialized, length)
    end
end
